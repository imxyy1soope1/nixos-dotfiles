...
Item {
        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    ...
                    if (line === "" || line.includes("ERROR") || line.includes("WARN") || line.includes("error:") || line.includes("warning:")) {
                        return;
                    }
    ...
    function updateModelIfChanged(newEntries) {
        ...
        for (let i = 0; i < newEntries.length; i++) {
            if (i >= currentEntries.length || newEntries[i].id !== currentEntries[i].id || newEntries[i].content !== currentEntries[i].content) {
                hasChanges = true;
                break;
            }
        }
    ...
    function detectContentType(content) {
        ...
        if (/^https?:\/\/\S+$/.test(content.trim()))
            return "url";
        // Code detection
        if (content.includes('\n') && (content.includes('{') || content.includes('function') || content.includes('=>')))
            return "code";
        // Command detection
        if (content.startsWith('sudo ') || content.startsWith('pacman ') || content.startsWith('apt '))
            return "command";
        return "text";
    }
